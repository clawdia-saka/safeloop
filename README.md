...
Runtime firewall policy profiles are specified as `strict-local`, `agent-dev`, and `ci-readonly` in [`docs/specs/runtime-tool-firewall-policy-profiles-v1.md`](docs/specs/runtime-tool-firewall-policy-profiles-v1.md). Profiles name the route posture and shim coverage v2 expectations; operator packets should surface the active firewall policy profile, shim coverage version, partial or missing coverage, and the PATH bypass caveat.

`safeloop watch-run --tool-shims` adds a run-local PATH fence around common risky tools. SafeLoop writes shims to `RUN_DIR/tool-shims/bin/`, prepends that directory to the watched command's `PATH`, and routes command-name lookups for local mutation tools, external tools, Git, Python, shell, and common JavaScript runners through `safeloop firewall exec`. Read-only commands can execute when the active policy profile allows them; destructive local requests are quarantined outside `ci-readonly`, external write/send/publish requests go to the outbox outside `ci-readonly`, and unknown semantics require manual review. The operator packet reports whether tool shims were enabled. Caveat: PATH shims intercept command-name lookups only; absolute executable paths and already-running processes can bypass them.

```bash
safeloop watch-run --tool-shims --policy-profile agent-dev --task-id demo --repo "$PWD" -- python agent.py
```
