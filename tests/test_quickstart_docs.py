...
def test_readme_has_public_one_line_install_and_e2e_quickstart() -> None:
    ...
    assert 'pipx install git+https://github.com/clawdia-saka/safeloop.git@v0.1.4' in readme
    ...
    assert 'safeloop verify-artifacts "$RUN_DIR"  # valid; notes intentional rollback-restore-source-drift, not packet tamper' in readme
